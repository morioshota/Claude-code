...
- `src/components/` — UI部品（`ui.jsx` 共通部品 / `DexCard` / `DetailModal` / `StockForm` / `notes` / `AiAssistant` / `modals`（パーティ・実績・バックアップ）/ `Ranch`（カイロ風2Dアイソメ牧場）/ `Album`（卒業アルバム）/ `Heatmap`（草カレンダー）/ `TriggerCheck`（トリガー点検）/ `fx`（演出レイヤー・セレモニー））
