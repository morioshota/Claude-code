...
- **カイロ風牧場**: `Ranch.jsx`。canvas 2Dの自前アイソメエンジン（タイル24×12px・アートピクセルを整数ズームで拡大・`imageSmoothingEnabled=false`）。銘柄ごとに「研究小屋」が建ち研究ステージで成長（ST1小屋→ST4御殿+旗。屋根はタイプ色、夜はまどが灯る、冬は屋根に雪）。保有銘柄は家の横のはたけで働き、ウォッチはさんぽ、鮮度90日超は家の前でおひるね（不変条件3）。吹き出しはオーナー自身のメモ（仮説・わざ・トリガー・クイック記録）の復習。含み損益の「ようす」で✨/💧と足どりが変わる（事実の写像・不変条件5の例外）。操作: ドラッグ=パン、＋−/ホイール=ズーム、タップ=詳細（7px閾値）。描画: 地面は静的キャンバスにベイク、建物/木/クリーチャーは奥行き(i+j)ソートで毎フレーム合成。建物・スプライトはMapでキャッシュ
