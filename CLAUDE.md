...
- **カードのホロ演出（ポケポケ風）**: `lib/cardfx.js` ＋ `DexCard.jsx`。カードごとにイベントを張らず、**1つのrAFループ**が各カードへCSS変数（`--tx/--ty`=傾き、`--gx/--gy`=光沢の中心、`--hp`=虹の位置）を書き込む。入力は**スクロール位置**（既定・許可不要）＋**指/カーソル**＋**端末の傾き**（📱ボタンで有効化）。
  ...
  - ホロの強さは4段階（`holoTierOf`: レアリティか研究ステージの高いほう）。**⚠ 虹を全面に出すとカードの黒基調が飛んで別物になる**——`kzHoloSheet`は`mask-image`で「光の当たっている所」だけに限定し、不透明度も低く抑えている
