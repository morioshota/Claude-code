...
- **カイロ風牧場**: `Ranch.jsx`。canvas 2Dの自前アイソメエンジン（タイル24×12px・アートピクセルを整数ズームで拡大・`imageSmoothingEnabled=false`）。**銘柄ごとに柵で囲まれた敷地**があり、クリーチャーは基本自分の敷地内で暮らす（ときどき前のみちへおでかけ→ご近所挨拶♪）。研究所は**研究ステージで形が変わる**: ST1テント→ST2小屋→ST3ラボ（別館+アンテナ）→ST4御殿（塔+旗+金帯）。屋根はタイプ色、夜はまどが灯る、冬は屋根に雪。**含み損益(事実)で研究所と敷地の大きさが変わる**（`boostOf`: 含み損≤-15%で0.75倍〜含み益は+50%ごとに約+1倍で青天井。0.1刻みに量子化してシーン再構築を抑制）。保有銘柄ははたけ仕事（**作物はnoteCountで育つ**: 1件=芽/3件=生育/6件=実り。かざりのみ）、ウォッチはさんぽ、鮮度90日超はおひるね（不変条件3）。クイックメモの日付が21日以内なら研究所に🗓が浮かび吹き出しでも喋る（`lib/events.js`）。吹き出しはオーナー自身のメモの復習。ようす（✨/💧・足どり）は含み損益の事実の写像（不変条件5の例外）。**1日ダイジェスト**: 牧場上部のニュースティッカー（📰）に🗓予定/🔔点検/🥀風化/✨💧ようす/🔥連続を巡回表示（事実のみ）。操作: ドラッグ=パン、＋−/ホイール=ズーム、タップ=詳細（7px閾値）。描画: 地面は静的キャンバスにベイク、建物/木/クリーチャーは奥行き(i+j)ソートで毎フレーム合成、建物・スプライトはMapでキャッシュ。⚠ `shadeHex`は#hex専用（rgb文字列を渡すと無効色になり前の描画色が残るバグを踏んだ）
