...
| セクション | 主な関数 / 変数 | 役割 |
|---|---|---|
| 定数・状態 | `PAPER`, `A3_WIDTH_MM`, `LS_KEY`, `state`, `uid`, `pxPerM`, `mPerPxFromDenom`/`denomFromMPerPx` | 用紙寸法、全体状態、ID 採番、縮尺換算（縮尺分母↔mPerPx 含む） |
| **パターン管理** | `makePattern`, `activePat`, `syncActivePattern`, `loadActivePattern`, `activatePattern`, `addPattern`, `deletePattern`, `renamePattern`, `normalizePattern`, `wrapAsPattern`, `frameView`, `frameFromDrag` | **複数パターン（v2.0）**。`state.objects` とアクティブパターンの同期、出力範囲枠の幾何 |
...
| キャンバス/ビュー | `resize`, `toWorld`, `evPos`, `zoomFit`, `measCtx`/`measureLabel`, `symbolLabelGeom`/`drawSymbolLabel` | 画面⇔世界座標変換、ズーム調整、記号ラベル幾何・描画（v1.3） |
| 描画 | `render`, `drawSheet`, `drawObj`, `conePositions`, `drawDim` | **毎フレームの描画中枢**。`drawSheet` は「地図レイヤ(opt.view でクロップ/ズーム可)＋用紙クローム(図枠/表題/凡例/スケールバー)」に分離。`drawObj` が type ごとに分岐 |
| 凡例・スケールバー | `usedLegendItems`, `drawLegend`, `drawLegendIcon`, `niceBarMeters`, `drawScaleBar(c,mPerPx)` | 使用記号から凡例を自動生成、用紙左下のスケールバー（実効縮尺 mPerPx で描画） |
| 選択(複数対応) | `selectedObjs`, `isSel`, `setSel`, `toggleSel`, `clearSel`, `selectLegend`, `objsInRect`, `normRect`, `rectsIntersect`, `moveObjBy`, `shiftObj` | **複数選択（v2.1）**。`state.selIds` が選択の実体、`sel` は単一選択/凡例のみ |
| 選択・オーバーレイ | `objBBox`, `objPoints`, `drawOverlays` | 選択枠(複数)・頂点ハンドル(単一時のみ)・マーキー矩形・作図中プレビュー・**出力範囲枠ガイド** |
| ヒットテスト | `hitTest`, `hitVertex` | クリック位置のオブジェクト/頂点特定（記号ラベル箱も対象） |
| Undo/Redo/自動保存 | `snapshot`, `pushUndo`, `undo`, `redo`, `applySnap`, `autosave`, `showToast` | 履歴管理（パターン内限定）と localStorage 保存（圧縮背景・容量超過フォールバック＋トースト） |
| ツール/ヒント | `setHint`, `setTool` | 現在ツールの切替とヒント文言（`frame`=出力範囲指定 を含む） |
| 入力処理 | `pointerDown/Move/Up`, `finishDraft`, keydown ハンドラ | **マウス・キーボード操作の全処理**（`frame` ツール=2点で範囲枠） |
| 縮尺設定 | `finishCalib`(async), `updateScaleBadge`, `niceDenom`, `openCalibModal` | 2 点+実距離／縮尺分母直接入力から `mPerPx` を決定 |
| 規制パターン自動配置 | `runTemplate(A,B)`, `wizardParams` | 3 パターンの部材一括生成（※「規制パターン」＝自動配置。「パターン」タブとは別概念） |
| パレット構築 | `PALETTE`, `buildPalette`, `drawToolIcon` | 左パレットの定義と生成 |
| パターンバー | `renderPatternBar` | 上部のパターンタブ・追加/複製/名前変更/削除・出力範囲ボタン |
| プロパティパネル | `renderProps` | 選択物に応じた右パネル UI を生成・バインド（記号ラベル欄含む） |
| 背景読込 | `ensurePdfJs`, `loadBackground`, `setBgFromDataURL`, `makeSaveURL` | PDF/画像の取り込み（PDF.js 遅延ロード、複数ページ選択、圧縮 saveURL 生成） |
| 保存/読込/出力 | `saveJson`, `loadJson`, `renderExportCanvas`, `exportPng`, `doPrint` | JSON 入出力（v2/patterns、v1 自動移行）・PNG・印刷（範囲枠でクロップ出力） |
| UIバインド | `syncSettingsUI`, `bindUI` | ボタン/入力の onclick 等を接続 |
| 初期化 | `init` | 起動時の復元（v2/v1）・既定パターン生成・パレット構築・初回フィット |
